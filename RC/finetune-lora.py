...
def train():
    ...
    logger.info("**********开始训练**********")
    if training_args.do_train:
        print(f"Restarting from --------------------------------2")
        checkpoint = None
        if training_args.resume_from_checkpoint is not None:
            print(f"Restarting from --------------------------------3")
            resume_from_checkpoint = training_args.resume_from_checkpoint
            checkpoint_name = os.path.join(
                resume_from_checkpoint, "pytorch_model.bin")
            print(f"Restarting from --------------------------------4")
            ...
            print(f"Restarting from --------------------------------1")
        if torch.__version__ >= "2" and sys.platform != "win32":
            model = torch.compile(model)
            print(f"Restarting from --------------------------------5")
        #train_result = trainer.train(resume_from_checkpoint=True)
        train_result = trainer.train(resume_from_checkpoint=checkpoint)
        #train_result = trainer.train()
        print(f"Restarting from --------------------------------9")
        trainer.save_model()  # Saves the tokenizer too for easy upload

        metrics = train_result.metrics
        print(f"Restarting from --------------------------------7")
        max_train_samples = (
            data_args.max_train_samples if data_args.max_train_samples is not None else len(
                train_dataset)
        )
        print(f"Restarting from --------------------------------8")
        metrics["train_samples"] = min(max_train_samples, len(train_dataset))
        print(f"Restarting from --------------------------------6")
